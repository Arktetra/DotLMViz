	...
	<div class="relative">
		{#if found}
			...
			<div
				...
				style="
                    ...
                    left: {found.x > 0.8 * global_state.tokens.length
					? xScale(found.x) - 100 + box_width
					: xScale(found.x) + box_width}px;
                    display: {visible ? 'block' : 'none'};
                "
			>
		...
		<svg {width} {height}>
			...
			<text
				...
				dy="15"
				text-anchor="middle"
				transform="rotate(0, {xScale(0)}, {yScale(global_state.tokens.length / 2)})">{xlabel}</text
			>
		</svg>
	</div>
</div>
